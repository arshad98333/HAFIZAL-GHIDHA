...
async def _latest_cell_f1(book: lb.Logbook) -> dict[str, float]:
...
async def score_cells(wave: int, book: lb.Logbook, settings: Settings | None = None) -> dict[str, float]:
...
async def build_plan(wave: int, book: lb.Logbook, settings: Settings | None = None) -> dict[str, Any]:
    settings = settings or get_settings()
    ...
    artifacts = focus.get("artifacts") or lb.ARTIFACTS
